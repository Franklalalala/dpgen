    ...
    for ii, ss in zip(init_data_sys_, init_batch_size_) :
        if jdata.get('init_multi_systems', False):
            for single_sys in os.listdir(os.path.join(work_path, 'data.init', ii)):
                init_data_sys.append(os.path.join('..', 'data.init', ii, single_sys).replace('\\', '/'))
                init_batch_size.append(detect_batch_size(ss, os.path.join(work_path, 'data.init', ii, single_sys).replace('\\', '/')))
        else:
            init_data_sys.append(os.path.join('..', 'data.init', ii).replace('\\', '/'))
            init_batch_size.append(detect_batch_size(ss, os.path.join(work_path, 'data.init', ii).replace('\\', '/')))
    old_range = None
    if iter_index > 0 :
        for ii in range(iter_index) :
            if ii == iter_index - 1:
            ...
            fp_data_sys = glob.glob(os.path.join(fp_path, "data.*").replace('\\', '/'))
            for jj in fp_data_sys :
                ...
                if jdata.get('use_clusters', False):
                    ...
                    if nframes < fp_task_min :
                        ...
                        continue
                    for sys_single in os.listdir(jj):
                        init_data_sys.append(os.path.join('..', 'data.iters', jj, sys_single).replace('\\', '/'))
                    ...
                    if nframes < fp_task_min :
                        log_task('nframes (%d) in data sys %s is too small, skip' % (nframes, jj))
                        continue
                    init_data_sys.append(os.path.join('..', 'data.iters', jj).replace('\\', '/'))
